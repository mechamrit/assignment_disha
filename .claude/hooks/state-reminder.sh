#!/usr/bin/env bash
# Stop hook (advisory): when the working tree has uncommitted code changes and docs/STATE.md
# is not among them, show a reminder to update the ledger. Never blocks stopping.
set -uo pipefail
...
# Only docs or personal notes changed: nothing to remind.
grep -qvE '^.. (docs/|CLAUDE\.local\.md$)' <<<"$changes" || exit 0
